...
from src.database.input_validation import input_bool, input_int
import json
import os
...
def importar_json_generico(
    ...
    print(f"Iniciando a importação de dados para a tabela de {model.display_name_plural()}")
    ...
    if len(arquivos_json) == 0:
        logging.warning(f"Nenhum arquivo encontrado no diretório de importação {IMPORTS_DIR}")
        return

    logging.info(f"Arquivos encontrados no diretório de importação: {len(arquivos_json)}")
        ...
        match escolha:
            ...
            case _:
                if escolha > len(arquivos_json):
                    logging.warning(f"Escolha inválida. Escolha um número entre 1 e {len(arquivos_json)}")
                    escolha = None
    ...
    if isinstance(data, dict):
        try:
            ...
            logging.error(f"Erro ao importar o arquivo {arquivo_target}: {e}\nVerifique se o arquivo está no formato correto.\nExemplo de instância: {exemplo.to_json()}")
            return
        ...
        exemplo = model.exemple_instance()
        logging.error(f"Erro ao importar o arquivo {arquivo_target}: o arquivo não está no formato JSON válido.\nVerifique se o arquivo está no formato correto.\nExemplo de instância: [\n{exemplo.to_json()}\n...\n]")
        return

    if len(instances) == 0:
        logging.warning(f"Nenhuma instância encontrada no arquivo {arquivo_target}")
        return
