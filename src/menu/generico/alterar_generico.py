...
def alterar_generico(
    ...
    while id_alterar not in ids_target:
        ...
        if id_alterar == 0:
            logging.info('Voltando para o menu anterior...')
            return

        if id_alterar not in ids_target:
            logging.warning(f'{model.display_name()} com ID {id_alterar} não está presente na base de dados, tente novamente!')

    instance = model.fetch_by_id(id_alterar)

    logging.info(f'Alterando {model.display_name()} com ID {instance.id}...')
