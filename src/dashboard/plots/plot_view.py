...
class PlotView:
    ...
    def view(self):
        ...
        if sensor_selecionado is not None and data_inicial is not None and data_final is not None:
            leituras = get_leituras_for_sensor(sensor_selecionado.id, data_inicial, data_final)

            if len(leituras) > 0:
                self.get_grafico(sensor_selecionado, leituras, f"Gráfico de {self.tipo_sensor} do sensor {sensor_selecionado.nome}")
            else:
